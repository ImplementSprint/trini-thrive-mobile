...
interface Campaign {
  ...
  description: string;
  status: string;
  sitemanager_notes?: string | null;
  participant_remaining?: number | null;
}
...
export default function PledgeScreen() {
  ...
  // --- DATA ARRAYS ---
  const selectedCampaign = campaigns.find(c => c.id === selectedSite);
  const selectedCampaignTitle = selectedCampaign?.name || "Select Site Location";
      ...
      {showSimpleSuccessModal && (
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { alignItems: 'center', padding: 40 }]}>
            ...
            >
              <Text style={{ color: '#FFFFFF', fontSize: 16, fontWeight: 'bold' }}>Return to Dashboard</Text>
            </Pressable>
          </View>
        </View>
      )}
        ...
        <View style={styles.contentWrapper}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
            ...
            <View style={styles.section}>
              <Text style={styles.fieldLabel}>Select Site Location <Text style={styles.requiredAsterisk}>*</Text></Text>
              <Pressable
                ...
                <View style={styles.dropdownMenu}>
                  {campaigns.map((campaign, index) => (
                    <Pressable key={index} style={styles.dropdownItem} onPress={() => { setSelectedSite(campaign.id); setIsSiteDropdownOpen(false); }}>
                      ...
                      {campaign.participant_remaining !== null && campaign.participant_remaining !== undefined && (
                        <Text style={styles.dropdownMetaText}>{campaign.participant_remaining} donor slot{campaign.participant_remaining === 1 ? '' : 's'} remaining</Text>
                      )}
                      {campaign.sitemanager_notes && (
                        <Text style={styles.dropdownNoteText}>{campaign.sitemanager_notes}</Text>
                      )}
                    </Pressable>
                  ))}
                  {campaigns.length === 0 && (
              ...
              <Text style={[styles.fieldLabel, { marginTop: 20 }]}>Select Time Slot <Text style={styles.requiredAsterisk}>*</Text></Text>
              <Pressable
            ...
            {/* 2. ITEMS TO DONATE */}
            <View style={styles.section}>
              <Text style={styles.fieldLabel}>Input Donation Item Details <Text style={styles.requiredAsterisk}>*</Text></Text>
              ...
              <Text style={[styles.fieldLabel, { marginTop: 20 }]}>Donor Message</Text>
              <TextInput
                style={styles.messageBox}
                value={donorMessage}
                onChangeText={setDonorMessage}
                placeholder="Add notes, concerns, or a message"
                multiline
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  requiredAsterisk: { color: '#E53E3E', fontSize: 16, fontWeight: 'bold' },
  ...
  messageBox: { minHeight: 80, backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 14, color: '#111827', textAlignVertical: 'top' },
