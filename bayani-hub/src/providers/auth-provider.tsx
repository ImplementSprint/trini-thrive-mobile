  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const storedUser = await getStorageItemAsync(USER_KEY);
        ...
        await deleteStorageItemAsync(USER_KEY);
      } finally {
        setIsReady(true);
      }
    })();
  ...
  const value = useMemo<AuthContextValue>(
    () => ({
      ...
      login: async (email: string, password: string) => {
        ...
        await setStorageItemAsync(TOKEN_KEY, data.access_token);
        await setStorageItemAsync(USER_KEY, JSON.stringify(nextUser));
      },
